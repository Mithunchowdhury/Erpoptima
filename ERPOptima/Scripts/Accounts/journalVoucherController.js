app.controller("journalVoucherController", function ($scope, $timeout, $modal, $http, Erpmodal) {
    ...
    $scope.table = [];
    $scope.Voucher = { Date: "", Type: 3, CmnBusinessesId: 1, VoucherNumber: "", Naration: "", VouchrerDetails: $scope.table, VoucherSerial: 0, TotalAmount: 0 };
    $scope.minDate;
    $scope.maxDate;
    $scope.ShowOrHide;
    ...
    $scope.GetVoucherNo = function () {
        $http({
            ...
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            data: { Type: 3, Prefix: "JV", CmnBusinessId: $scope.Voucher.CmnBusinessesId, Date: $scope.Voucher.Date }
        }).success(function (data) {
            $timeout(function () {
    ...
    $scope.Voucher = { Date: $scope.Voucher.Date, Type: 3, CmnBusinessesId: 1,VoucherNumber: "", Naration: "", VouchrerDetails: $scope.table, VoucherSerial: 0, TotalAmount: 0 };
    $scope.Count = 1;
        ...
        Erpmodal.Confirm('Save').then(function (result) {
            if ($scope.table != "") {
                if ($scope.DebitCal() != $scope.CreditCal()) {
                    ...
                    $scope.Voucher = { Id: $scope.Id, CmnBusinessId: $scope.Voucher.CmnBusinessesId, Date: $scope.Voucher.Date, Type: 3, VoucherNumber: $scope.Voucher.VoucherNumber, Naration: $scope.Voucher.Naration, VouchrerDetails: $scope.table, VoucherSerial: $scope.Count, TotalAmount: $scope.TotalAmount };
    ...
    $scope.searchVouchersById = function (obj) {
        $http({
            ...
            $scope.Voucher = { Date: obj.DateString, CmnBusinessesId: obj.CmnBusinessId, Type: 3, VoucherNumber: obj.VoucherNumber, Naration: obj.Naration, VouchrerDetails: $scope.table, VoucherSerial: $scope.Count, TotalAmount: $scope.TotalAmount };
...
var ModalInstanceCtrl = function ($scope, $timeout, $http, $modalInstance, search) {
    ...
    $scope.Clear = function () {
        ...
        $scope.search.toDate = '';
        $scope.search.CmnBusinessesId = 0;
        $scope.Projects = {};
        $scope.search.ProjectId = 0;
        $scope.search.CostCenterId = 0;
    };
    $scope.load = function () {
        $http({
            url: '/Accounts/Voucher/SearchByItems',
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            data: { VoucherTypeId: 3, VoucherNumber: search.VoucherNumber, DateFrom: search.fromDate, ToDate: search.toDate, ProjectId: search.ProjectId, CostCenterId: search.CostCenterId }
        }).success(function (data) {
            $timeout(function () {
                $scope.result = data;
                $scope.$apply();
