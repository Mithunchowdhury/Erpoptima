app.controller("receiptVoucherController", function ($scope, $timeout, $modal, $http, Erpmodal) {
    ...
    $scope.GetVoucherNo = function () {
        $http({
            ...
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            data: { Type: 2, Prefix: "RV", CmnBusinessId: $scope.Voucher.CmnBusinessesId, Date: $scope.Voucher.Date }
        }).success(function (data) {
            $timeout(function () {
        ...
        Erpmodal.Confirm('Save').then(function (result) {
            if ($scope.table != "") {
                if ($scope.DebitCal() != $scope.CreditCal()) {
                    ...
                    $scope.Voucher = { Id: $scope.Id, CmnBusinessId: $scope.Voucher.CmnBusinessesId, Date: $scope.Voucher.Date, Type: 2, VoucherNumber: $scope.Voucher.VoucherNumber, Naration: $scope.Voucher.Naration, VouchrerDetails: $scope.table, VoucherSerial: $scope.Count, TotalAmount: $scope.TotalAmount };
    ...
    $scope.searchVouchersById = function (obj) {
        $http({
            ...
            $scope.Voucher = { Date: obj.DateString, CmnBusinessesId: obj.CmnBusinessId, Type: 2, VoucherNumber: obj.VoucherNumber, Naration: obj.Naration, VouchrerDetails: $scope.table, VoucherSerial: $scope.Count, TotalAmount: $scope.TotalAmount };
...
var ModalInstanceCtrl = function ($scope, $timeout, $http, $modalInstance, search) {
    ...
    $scope.load = function () {
        $http({
            ...
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            data: { VoucherTypeId: 2, VoucherNumber: search.VoucherNumber, DateFrom: search.fromDate, ToDate: search.toDate, ProjectId: search.ProjectId, CostCenterId: search.CostCenterId }
    }).success(function (data) {
            $timeout(function () {
                $scope.result = data;
                $scope.$apply();
